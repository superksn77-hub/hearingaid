...
// Gemini API는 선택사항 — 규칙 기반 전문 분석이 기본
...
export const HomeScreen: React.FC<Props> = ({ navigation, onGoToGate }) => {
  ...
  return (
    <ScrollView ref={scrollRef} style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* ── AI 분석 안내 (스크리닝 모드에서만 표시) ────────────── */}
      {testMode === 'screening' && (
        <View style={[styles.sectionCard, { paddingVertical: 12 }]}>
          <Text style={{ color: '#78909c', fontSize: 12, textAlign: 'center', lineHeight: 18 }}>
            검사 결과는 청각-인지 신경과학 기반 전문 분석 엔진이 자동으로 상세히 분석합니다.
          </Text>
        </View>
      )}
...
// Gemini API 설정 컴포넌트 제거 — 규칙 기반 전문 분석이 기본 제공
